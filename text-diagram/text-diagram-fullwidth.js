...
var html_render = (function() {
    ...
    for (var i = 0; i < meta.objs.length; ++i) {
      ...
      for (var j = 0; j < meta.notes[obj].length; ++j) {
        ...
        if ('left' == note_ast.attr.side) {
          if (i == 0) {
            var note_width = _note_width_with_full_width(note_ast.attr.content);
            var before_x1 = meta.boxes[obj].x1;
            var new_x1 = pre_line_offset + 1 + note_width + 1 - half_box_width;
            meta.boxes[obj].x1 = Math.max(meta.boxes[obj].x1, pre_line_offset + 1 + note_width + 1 - half_box_width);
            //console.log(obj, " 0 meta.boxes[obj].x1: ", meta.boxes[obj].x1, ", width: ", note_width, ", before_x1: ", before_x1, ", new_x1: ", new_x1);
          } else {
            var note_width = _note_width_with_full_width(note_ast.attr.content);
            meta.boxes[obj].x1 = Math.max(meta.boxes[obj].x1, pre_line_offset + 1 + note_width + 1 - half_box_width);
          }
      ...
      if (i > 0) {
        ...
        for (var k = 0; k < meta.notes[pre_obj].length; ++k) {
          ...
          if ('right' == note_ast.attr.side) {
            meta.boxes[obj].x1 = Math.max(meta.boxes[obj].x1, meta.lines[pre_obj].x_offset + 1 + _note_width_with_full_width(note_ast.attr.content));
            //console.log(obj, " 1 meta.boxes[obj].x1: ", meta.boxes[obj].x1, ", width: ", _note_width_with_full_width(msg_ast.attr.message));
          }
        }
      }
      ...
      for (var j = 0; j < i; j++) {
        ...
        for (var k = 0; k < meta.messages[pre_obj].length; ++k) {
          ...
          if (msg_ast.meta.right_obj == obj) {
            var before_x1 = meta.boxes[obj].x1;
            var new_x1 = meta.lines[pre_obj].x_offset + 1 + _note_width_with_full_width(msg_ast.attr.message);
            meta.boxes[obj].x1 = Math.max(meta.boxes[obj].x1, meta.lines[pre_obj].x_offset + 1 + _note_width_with_full_width(msg_ast.attr.message));
            //console.log(obj, " 2 meta.boxes[obj].x1: ", meta.boxes[obj].x1, ", width: ", _note_width_with_full_width(msg_ast.attr.message), ", before_x1: ", before_x1, ", new_x1: ", new_x1);
          }
        }
      }
      ...
      if (i > 0) {
        ...
        for (var j = 0; j < meta.messages[pre_obj].length; ++j) {
          ...
          if (tmp_ast.meta.sender_index == tmp_ast.meta.receiver_index) {
            meta.boxes[obj].x1 = Math.max(meta.boxes[obj].x1, meta.lines[pre_obj].x_offset + 1 + _msg_width_with_full_width(tmp_ast.attr.message));
            //console.log(obj, " 3 meta.boxes[obj].x1: ", meta.boxes[obj].x1, ", width: ", _msg_width_with_full_width(msg_ast.attr.message));
          }
        }
      }
      ...
      //console.log(obj, " meta.lines[obj].x_offset: ", meta.lines[obj].x_offset);
      ...
      for (var j = 0; j < meta.notes[obj].length; ++j) {
        ...
        if ('right' == note.side) {
          var note_width = _note_width_with_full_width(note);
      ...
      for (var j = 0; j < meta.messages[obj].length; ++j) {
        ...
        if (tmp_ast.meta.sender_index == tmp_ast.meta.receiver_index) {
          var message_width = _msg_width_with_full_width(tmp_ast.attr.message);
  ...
  function _to_cimage(in_ast) {
    ...
    for (var i in meta.statements) {
      ...
      if ('message_statement' == ast.type) {
        ...
        if ('right' == side) {
          ...
          if ( meta.obj_idxes[obj] == 0 ) {
            _draw_cpoints(ccanvas, 0, ast.meta.y1, cnote);
          } else {
    ...
    for (var i in meta.objs) {
      var obj = meta.objs[i];
      var cline = _lifeline(meta.lines[obj].x_offset - meta.min_x, meta.height, 3, ccanvas);
      _draw_cpoints(ccanvas, 0, 3, cline);
    }
  ...
  function _draw_cpoints(in_canvas, in_x_offset, in_y_offset, in_cpoints) {
    for (var i in in_cpoints) {
      var p = in_cpoints[i];
      //console.log("c: ", p.c + ", x: " + (in_x_offset + p.x) + ", y: " + (in_y_offset + p.y));
  ...
  function _note_width_with_full_width(msg) {
    var content = ('string' == typeof(msg) ? msg : msg.attr.content);
    var lines = content.split('\\n');
    var max = 0;
    for (var i = 0; i < lines.length; ++i) {
        const w = _getDisplayWidth(lines[i].trim());
        if (w > max) {
          max = w;
        }
    }
  ...
  function _msg_width_with_full_width(msg) {
    var lines = msg.split('\\n');
    var max = 0;
    for (var i = 0; i < lines.length; ++i) {
        const w = _getDisplayWidth(lines[i].trim());
  ...
  function _cnote(msg, is_left) {
    var i;
    var x = _note_width_with_full_width(msg);
    var y = _note_height(msg);
    var hasNotFullWidthCharacter = _getFullWidthCharacterCount(msg) == 0;

    var out_cimage = [];

        //association line
        if (is_left) {
            out_cimage.push(_cpoint('-', x, 2, 0));
            xoffset = 0;
        }
        else {
    ...
    for (i = 0; i < y - 1; ++i) {
        ...
        if(hasNotFullWidthCharacter || i == y - 2) {
    ...
    if (isSelfMessage) {
      ...
      for(var idx = 0; idx < lines.length; idx++) {
        ...
        for (var i = 0; i < line.length; ++i) {
          cpoints.push(_cpoint(line.charAt(i), 1 + i, 1 + idx, 0));
        }
      }
      ...
      for(var idx = 0; idx < lines.length; idx++) {
        ...
        const w = _getDisplayWidth(line);
        for (var i = 0; i < w; ++i) {
          cpoints.push(_cpoint(lines[idx].charAt(line.length - 1 - i), line_len - 1 - i - 1, 1 + idx, 0));
        }
      }
  ...
  */
  ...
  function _lifeline(in_x_offset, in_height, headerCount, ccanvas) {
    var cline = [];
    for (var j = 0; j < in_height - headerCount; ++j) {
      if(_is_empty(in_x_offset, j + headerCount, ccanvas)) {
        const leftFullWidthCharacterCount = _find_left_full_width_character_count(in_x_offset, j + headerCount, ccanvas);
        if( leftFullWidthCharacterCount == 0 ) {
          cline.push(_cpoint('|', in_x_offset, j, 0));
        }
      }
    }
    return cline;
  ...
  function _is_empty(x, y, ccanvas) {
    return !ccanvas[y][x];
  }

  function _find_left_full_width_character_count(x, y, ccanvas) {
    var count = 0;
    for (var i = x; i >= 0; i--) {
        if (ccanvas[y][i] && ccanvas[y][i].c.charCodeAt(0) > 255) {
            count++;
        }
    }
    return count;
  ...
  // 문자열의 실제 출력 폭
  function _getDisplayWidth(str) {
    let width = 0;
    for (const ch of str) {
      // (더 정교하게 처리하려면 Unicode East Asian Width 등의 표준 판정 로직 사용)
  ...
  function _getFullWidthCharacterCount(str) {
    let count = 0;
    for (const ch of str) {
      // (더 정교하게 처리하려면 Unicode East Asian Width 등의 표준 판정 로직 사용)
      if (ch.charCodeAt(0) > 255) {
